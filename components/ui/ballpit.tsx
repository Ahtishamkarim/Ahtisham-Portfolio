...
function getSafeWebGL2Context(canvas: HTMLCanvasElement): WebGL2RenderingContext {
  const gl = canvas.getContext("webgl2", WEBGL_CONTEXT_ATTRIBUTES);
  if (!gl || gl.isContextLost()) {
    throw new Error("WebGL2 is not available");
  }

  // Some browsers (and lost/reused canvases) return null from
  // getShaderPrecisionFormat; Three.js then crashes reading `.precision`.
...
class X {
  ...
  #initRenderer() {
    ...
    getSafeWebGL2Context(this.canvas);
  ...
  updateWorldSize() {
    ...
    } else if ((this.camera as PerspectiveCamera & { isOrthographicCamera?: boolean }).isOrthographicCamera) {
      const cam = this.camera as PerspectiveCamera & { top: number; bottom: number; right: number; left: number };
  ...
  clear() {
    this.scene.traverse((obj) => {
      const mesh = obj as { isMesh?: boolean; material?: Record<string, { dispose?: () => void }> & { dispose?: () => void }; geometry?: { dispose?: () => void } };
...
const Ballpit: React.FC<BallpitProps> = ({ className = "", followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // Defer so React Strict Mode's immediate setup/cleanup does not create
    // a WebGL context and then reuse a lost canvas on the second mount.
    frame = requestAnimationFrame(() => {
      ...
      try {
        ...
        console.error("Ballpit failed to start", error);
      }
    });
